...
const SpidermanNoWayHomePage = () => {
    const [selectedVideo, setSelectedVideo] = useState(null);
    const [videos, search] = useVideos('Spiderman No Way Home Trailer');
    const [data, setData] = useState([])
    const [loading, setLoading] = useState(true)
    const [videoLoading, setVideoLoading] = useState(true)
    const bookingFormURLParam = 'spiderman_no_way_home'
    ...
    const getData = async () => {
        const res = await axios.get(`http://localhost:5000/movieRoutes/getById/5`)
        const data = res.data;
        setData(data);
        setLoading(false)
    }
...
export default SpidermanNoWayHomePage;