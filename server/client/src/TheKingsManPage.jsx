...
const TheKingsManPage = () => {
    const [selectedVideo, setSelectedVideo] = useState(null);
    const [videos, search] = useVideos("The King's Man Trailer");
    const [data, setData] = useState([])
    const [loading, setLoading] = useState(true)
    const [videoLoading, setVideoLoading] = useState(true)
    const bookingFormURLParam = 'the_kings_man'
    ...
    const getData = async () => {
        const res = await axios.get(`http://localhost:5000/movieRoutes/getById/7`)
        const data = res.data;
        setData(data);
        setLoading(false)
    }
...
export default TheKingsManPage;